...
if (!class_exists('recent_post_single')) {
  ...
  class recent_post_single extends WP_Widget
  {
    ...
    public function __construct()
    {
      parent::__construct(
        'recent-post-single',
        __('&hearts; Blogs Up - Recent Post', 'blogs-up'),
        array(
          'description' => __('(Recent Post) Displays latest posts or posts from a choosen category. ', 'blogs-up'),
...
?>
              ...
              while ($recent_list_posts->have_posts()) : $recent_list_posts->the_post(); ?>
                <article class="post-list">
                  <div class="post-thumb-outer">
                    <?php the_post_thumbnail('silkblog-small'); ?>
                  </div>
                  <div class="post-body">
    ...
    function form($instance)
    {
      ...
      <!-- Form for category 1 -->
      <h5><?php esc_html_e('News style', 'blogs-up'); ?></h5>
      <p>
        <label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title:', 'blogs-up'); ?></label>
        <input type="text" class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
      </p>
      <p>
        <label><?php esc_html_e('Select a post category', 'blogs-up'); ?></label>
        <?php $args = array(
      ...
      <p><input class="checkbox" type="checkbox" <?php checked($sticky_posts); ?> id="<?php echo $this->get_field_id('sticky_posts'); ?>" name="<?php echo $this->get_field_name('sticky_posts'); ?>" />
        <label for="<?php echo $this->get_field_id('sticky_posts'); ?>"><?php _e('Hide sticky posts.', 'blogs-up'); ?></label>
      </p>


      <p><label for="<?php echo $this->get_field_id('number_posts'); ?>"><?php _e('Number of posts to show:', 'blogs-up'); ?></label>
...
// register Blogs up dual category posts widget
function blogsup_recent_post_single()
{
  register_widget('recent_post_single');
}
add_action('widgets_init', 'blogsup_recent_post_single');
